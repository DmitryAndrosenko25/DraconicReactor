...
Это рабочий черновик разогрева реактора до рабочей температуры. Я его сохранил перед тем как основной файл чистить от лишнего говна
...
function reactorToWorkTemperature.startHeating(reactorAddress, fluxInAddress, fluxOutAddress, tempMax)
	...
	while tCurrent <= (tempMax) do 
		...
		if (tDelta < ((tempMax - tCurrent) / 200)) then -- 200!!!!
			...
			fluxOutGate.setFlowOverride(((rInfo("generationRate") * (tempMax / tCurrent)) + ((saturat /20) /100)) + 1) -- (rInfo("generationRate") * del))	-- /10!!! РОСТ МОЖЕТ БЫТЬ СЛИШКОМ БЫСТРЫЙ	
